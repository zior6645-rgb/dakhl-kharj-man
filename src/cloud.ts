...
export async function signUp(email:string,password:string):Promise<CloudSession|null>{const payload=await authFetch('signup',{method:'POST',body:JSON.stringify({email:email.trim().toLowerCase(),password})});return payload?.access_token ? saveCloudSession(toSession(payload)) : null;}
