...
این فایل «دستور ممیزی نهایی انتشار» پروژه Cashio است و باید در هر نسخه مهم قبل از انتشار دوباره اجرا شود.
...
## UI/Brand gate — نسخه 1.6
- نام نمایشی محصول: **Cashio**.
- شناسه Android نباید برای تغییر برند عوض شود تا نصب‌های قبلی قابل‌به‌روزرسانی بمانند.
- لوگو باید بدون حرف فارسی، متن یا نماد وابسته به یک کشور/ارز باشد و در اندازه کوچک نیز واضح بماند.
- صفحه Settings باید پنج سطح اندازه متن داشته باشد و مقدار انتخاب‌شده بین اجراها حفظ شود.
- صفحه Home عمداً ساده است و carousel/cardهای قابل‌جابه‌جایی ندارد؛ این مورد نباید دوباره به‌صورت خودکار اضافه شود.
- Home و Reports فقط از داده واقعی تراکنش استفاده کنند؛ داده ساختگی ممنوع.
- نمودارهای گزارش باید برای داده‌های نقدی/دخل‌وخرج مناسب باشند. اندیکاتورهای بازار سهام/رمزارز و OHLC ساختگی ممنوع‌اند؛ در عوض فقط نمودارها و شاخص‌های واقعی دخل‌وخرج نمایش داده شوند؛ نمودارهای بازار، کندل و اندیکاتورهای تکنیکال برای داده‌های بدون OHLC به‌صورت پیش‌فرض اضافه نشوند.
- RTL/LTR، اندازه متن و خوانایی لوگو باید در تمام زبان‌های پروژه بررسی شوند.
- متن‌ها نباید حالت offline-only را در حالت cloud القا کنند.
